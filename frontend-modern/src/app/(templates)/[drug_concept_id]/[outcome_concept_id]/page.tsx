...
export default async function Page({
  ...
  console.log(
    'Attempt tp load page for drug_concept_id: ' + params.drug_concept_id
  );
  const drugConceptId = Number(params.drug_concept_id);
  const outcomeConceptId = Number(params.outcome_concept_id);
  if (isNaN(outcomeConceptId)) return notFound();
  const data: Promise<DrugConditionDetailWrapper> =
    fetchDrugConditionDetailList(drugConceptId, outcomeConceptId).catch(
      (error) => {
        console.error(
          'Error occurred fetching drug condition detail list for: ' +
            drugConceptId
        );
        console.error(error);
      }
  ...
  let drugConceptDetailList: Promise<DrugConditionDetail[]> = data.then(
    (v) => v.sources
  );
    ...
    <PageWrapper>
      <main className='flex flex-col items-center gap-8 pt-8'>
        <Card className='w-full max-w-screen-xl p-16 text-center'>
          ...
          <p>
            Amongst patients taking Hydrochlorothiazide, onset of cardiac
            arrhythmia occurs in % to % of patients during the 1 year after
            starting the drug
          </p>
          <DrugConditionDetailsStackedBarChart
            data={data.then((v) => v.sources)}
          />
        </Card>
        <Card className='w-full max-w-screen-xl p-16 '>
          <DrugConditionDetailsTable
            id={drugConceptId}
            data={drugConceptDetailList}
