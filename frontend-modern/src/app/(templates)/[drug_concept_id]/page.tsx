...
export default async function Page({
  ...
  if (isNaN(drugConceptId)) return notFound();
  const data: Promise<DrugCondition[]> = fetchDrugConditionList(
    drugConceptId
  ).catch((error) => {
    console.error(
      'Error occurred fetching drug condition list for: ' + drugConceptId
    );
    console.error(error);
  });
    ...
    <PageWrapper>
      <main className='flex flex-col items-center gap-8 pt-8'>
        <Card className='w-full max-w-screen-xl p-16 text-center'>
          ...
          <p>
            Amongst patients taking Hydrochlorothiazide, onset of the following
